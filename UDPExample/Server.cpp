...
int main() {
	...
	while (true) {
		...
		if (FD_ISSET(server_socket, &readfds)) {
			...
			// 4 отправка сообщения о входе нового клиента всем клиентам
			string entryMessage = "New client joined!";
			SendToAllClients(entryMessage);
		...
		for (int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if (FD_ISSET(s, &readfds))
			{
				...
				if (check_exit == "off")
				{
					...
					// 4 отправка сообщения о выходе  клиента всем клиентам
					string leftMessage = "Client left!";
					SendToAllClients(leftMessage);
